...
| **Image — multiple (compositing)** | yes | ✅ **shipped 2026-07-26** | `image_urls` list on nano-banana/nb2/nbpro **/edit**; Seedream `images` list (**up to 15** total incl. outputs) | `GenOpts.extraImageUrls` + a per-model `maxInputs` cap; the composer's «Ещё ракурсы» strip adds extra angles of the same face. Server-set only — a client cannot inject arbitrary image urls. |
...
1. ✅ **Multi-image input — SHIPPED 2026-07-25** as a curated STYLE REFERENCE (`Preset.styleRef` / `CampaignPreset.styleRef` → `styleRefUrl()` → second entry in `image_urls`, on **all five** edit models — `premium_edit` was folded in 2026-07-25 once P7 confirmed its `image_urls` is an array). Two constraints that are load-bearing, not stylistic: the reference art must contain **no usable face** (a face invites identity blending, which breaks the product's one promise — the prompt also carries an explicit "style only, do not copy any person" clause), and the URL is resolved **server-side from the registry only** — assigned after `normalizeOpts`, which rebuilds opts from scratch and never copies it, so `/api/generate` can't be turned into a fetch-any-URL primitive. No extra provider cost (see the pricing note below). Originally scoped as: strong candidate for a fast-follow after Studio v1. Pricing note: Seedream counts input images toward its 15-item cap; nano-banana composites references into one output (no per-input surcharge beyond `num_images`).
   ...
   **Extended 2026-07-25 to USER-supplied multi-image input** (`ImageParams.maxInputs`, `GenOpts.extraImageUrls`, `/api/generate` `image_urls[]`, and the composer's "Ещё ракурсы" strip). Several angles of the SAME face is the cheapest available likeness win — the providers composite the references into one output, so the charge still follows `num_images` and extra angles are free. Caps are OURS, not the providers': 4 total on the cheap tiers, 2 on the premium tier where every input image is billed context. Three things are load-bearing:
   - `refPrompt` is **positional and composition-aware** — it states how many leading images are one person and that the style plate is LAST. Without it a model reads extra images as extra PEOPLE and returns a blend or a crowd, which is exactly the failure the product cannot ship.
   - `extraImageUrls` is server-set like `styleRefUrl`: `normalizeOpts` never copies it, and the handler validates every entry itself.
   - Client-named URLs are now restricted to `config.mediaHostSuffixes` (**this also tightened the pre-existing single-photo path**, which accepted any HTTPS URL — that was a way to skip the `/api/upload` moderation gate by hosting the photo elsewhere). Suffixes match on hostname boundaries, so `evil-fal.media` and `fal.media.attacker.com` both fail. Operational note: if the provider renames its CDN, generations fail with `bad_source` until `MEDIA_HOST_SUFFIXES` is updated — the reason it is config rather than a constant.
